...
export const DashboardPage: React.FC<Props> = ({ onNavigate }) => {
  ...
  if (loading) {
    return (
      <div>
        <h2 style={{ fontSize: '1.5rem', marginBottom: '20px' }}>Executive Safety Intelligence Dashboard</h2>
  ...
  return (
    <div>
      ...
      {/* Concept Alert Card */}
      <div className="glass-card" style={{ padding: '16px 20px', marginBottom: '24px', borderLeft: '4px solid var(--accent-cyan)', background: 'linear-gradient(135deg, rgba(0,141,218,0.1) 0%, rgba(13,27,46,0.8) 100%)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '28px' }}>
        ...
        <div className="glass-card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <button onClick={() => onNavigate('clusters')} className="btn btn-secondary" style={{ fontSize: '0.75rem', padding: '4px 10px' }}>
              View All Clusters
            </button>
          </div>
          ...
          {data.emerging_patterns.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {data.emerging_patterns.slice(0, 4).map((pattern, idx) => (
                <div key={pattern.id || idx} style={{ padding: '12px 14px', borderRadius: '8px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.06)' }}>
        ...
        <div className="glass-card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <button onClick={() => onNavigate('analytics')} className="btn btn-secondary" style={{ fontSize: '0.75rem', padding: '4px 10px' }}>
              Full Rankings
            </button>
          </div>
          ...
          {data.top_lsr.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {data.top_lsr.slice(0, 5).map((lsr, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: '8px', background: 'rgba(255, 255, 255, 0.02)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ width: '24px', height: '24px', borderRadius: '50%', background: 'rgba(0, 141, 218, 0.2)', color: 'var(--accent-cyan)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 700 }}>
                      {idx + 1}
                    </span>
                    <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>{lsr.rule_name}</span>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--accent-cyan)' }}>{lsr.count}</span>
                ...
                <tr key={idx}>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span>{(b.sif_density * 100).toFixed(1)}%</span>
                    </div>
                  </td>
                </tr>
              ))}
